...
export function ProjectDeepWikiConsole() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-bg-canvas text-text-main">
      <header className="shrink-0 border-b border-border-subtle bg-bg-surface">
        <div className="flex flex-col gap-4 px-4 py-4 lg:px-6">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0">
              <div className="flex min-w-0 flex-wrap items-center gap-1.5 text-xs text-text-muted">
                <Link href={ROUTES.CONSOLE.PROJECTS} className="hover:text-text-main">
                  Projects
                </Link>
                <span>/</span>
                <Link href={projectOverviewHref(projectId)} className="truncate hover:text-text-main">
                  {context.project.name}
                </Link>
                <span>/</span>
                <span className="font-medium text-text-main">DeepWiki</span>
              </div>
              <div className="mt-2 flex min-w-0 items-center gap-2">
                <BookMarked className="h-5 w-5 shrink-0 text-primary" />
                <h1 className="truncate text-xl font-semibold tracking-normal text-text-main">
                  {selectedRepository?.repository_id || 'Project DeepWiki'}
                </h1>
              </div>
          ...
          <ProjectDeepWikiNav projectId={projectId} />

          <div className="flex min-w-0 flex-col gap-3 xl:flex-row xl:items-center">
            <div className="min-w-0 flex-1">
              <div className="mb-1.5 text-xs font-medium text-text-muted">Repository</div>
              <div className="flex gap-2 overflow-x-auto pb-1">
                {deepWikiQuery.isLoading ? (
                  <div className="flex h-10 items-center gap-2 rounded-[4px] border border-border-subtle bg-bg-subtle px-3 text-sm text-text-muted">
...
function ProjectDeepWikiNav({ projectId }: { projectId: number }) {
  const items = [
    { label: 'Overview', href: projectOverviewHref(projectId) },
    { label: 'Context', href: projectContextHref(projectId) },
    { label: 'DeepWiki', href: projectDeepWikiHref(projectId), active: true },
    { label: 'Delivery', href: projectSpecForgeHref(projectId) },
  ];

  return (
    <nav className="flex gap-1 overflow-x-auto border-b border-border-subtle pb-2 text-sm">
      {items.map(item => (
        <Link
          key={item.label}
          href={item.href}
          className={cn(
            'flex h-8 shrink-0 items-center rounded-[4px] px-3 text-text-subtle hover:bg-bg-subtle hover:text-text-main',
            item.active && 'bg-primary-subtle font-medium text-primary hover:bg-primary-subtle hover:text-primary'
          )}
        >
          {item.label}
        </Link>
...
function RepositoryWikiItem({
  ...
  return (
    <button
      type="button"
      className={cn(
        'flex h-14 w-[280px] shrink-0 items-start gap-2 rounded-[4px] border border-border-subtle bg-bg-surface px-3 py-2 text-left hover:bg-bg-subtle',
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-1 flex flex-wrap items-center gap-2 text-xs text-text-muted">
          ...
          <span>{repositoryRoleLabel(repository.role)}</span>
