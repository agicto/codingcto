...
func NewDefaultRegistry(
	...
	for _, manifest := range DefaultManifests(auditHandler, apiKeyHandler, planningHandler, repoContextHandler, executionHandler, gitHubIntegrationHandler, projectHandler, verificationHandler, workspaceHandler, userHandler) {
...
func DefaultMigrations() (map[string]migration.Migration, error) {
	registry := NewRegistry()
	for _, manifest := range DefaultManifests(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil) {
		if err := registry.ApplyManifest(manifest); err != nil {
			return nil, err
		}
	}
