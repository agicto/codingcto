// Package redact masks credentials and local-only paths before text is
...
var patterns = []secretPattern{
	{regexp.MustCompile(`(?i)\b[A-Z0-9_]*(?:API_KEY|API_SECRET|SECRET_KEY|SECRET|ACCESS_TOKEN|AUTH_TOKEN|PRIVATE_KEY|DATABASE_URL|DB_PASSWORD|DB_URL|REDIS_URL|PASSWORD|TOKEN)\s*[=:]\s*\S+`), "[REDACTED CREDENTIAL]"},
...
// Text replaces known secrets in s with stable placeholders. Clean text is
// returned unchanged so callers can use it before every log persistence write.
...
// StringMap returns a copy of m with string values redacted.
