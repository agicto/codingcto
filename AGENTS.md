...
3. **Brand strings.** The user-facing brand is **CodingCTO**. Keep lower-case `luas` only where it is already part of compatibility identifiers such as package names, binary names, env keys, or historical module paths. Never re-introduce `LlamaFront`, `Hypership`, or `ZGO` into new user-facing code — those were old names and have been deliberately cleaned up.
